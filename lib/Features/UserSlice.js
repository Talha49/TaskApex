...
export const fetchTasks = createAsyncThunk('tasks/fetchTasks', async (userId) => {
  ...
  const res = await fetch(`/api/Tasks/GetUserid/${userId}`);
  if (res.ok) {
...
export const updateTask = createAsyncThunk(
  ...
  async ({ id, updateFields }) => {
    console.log('Updating task:', id, updateFields); // Corrected _id to id
    const res = await fetch('/api/Tasks/UpdateTask', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ id, updateFields }), // Correct structure
    });
...
// export const fetchTasksbyuserid = createAsyncThunk('tasks/FetchTasksbyuserid', async (userId) => {
//   const response = await fetch(`/api/Tasks/GetUserid/${userId}`);
//   if(response.ok){
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateTask.fulfilled, (state, action) => {
        const index = state.tasks.findIndex(task => task._id === action.payload._id); // Use _id
        if (index !== -1) {
