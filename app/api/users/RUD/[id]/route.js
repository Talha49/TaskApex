...
export async function PUT(req, { params }) {
  ...
  try {
    const { fullName, contact, team, role, password, tenantId } = await req.json();
    ...
    const updateData = { fullName, contact, team, role, tenantId, updatedBy: token.id, updatedAt: new Date() };
    if (password) updateData.password = await bcrypt.hash(password, 12);

    const updatedUser = await User.findOneAndUpdate(
      { _id: params.id, tenantId: token.tenantId },
      updateData,
      { new: true }
    );
    if (!updatedUser) return NextResponse.json({ error: "User not found", code: "NOT_FOUND" }, { status: 404 });
