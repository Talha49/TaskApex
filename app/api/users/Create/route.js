...
export async function POST(req) {
  ...
  try {
    const { fullName, email, password, contact, team, role } = await req.json();
    if (!fullName || !email || !password) {
      return NextResponse.json({ error: "Full name, email, and password are required", code: "VALIDATION_ERROR" }, { status: 400 });
    }

    const existingUser = await User.findOne({ email });
    if (existingUser) {
      return NextResponse.json({ error: "User already exists", code: "DUPLICATE_EMAIL" }, { status: 409 });
    }
    ...
    const user = new User({
      ...
      team: team || "",
      role,
      createdBy: token.id, // Audit trail
      createdAt: new Date(),
    });
